...
int main() {
    // Full-scale sine: RMS = 1/sqrt(2) ≈ -3.0103 dBFS, peak = 0 dBFS.
    {
        ...
        assert(near(lv.rms, 1.0 / std::sqrt(2.0), 1e-4));
        assert(near(lv.peak, 1.0, 1e-4));
        assert(near(toDbfs(lv.rms), -3.0103, 1e-2));
    }
    ...
    // Interleaved stereo: channel selection must not leak across channels.
    {
        ...
        assert(near(ch0.rms, 0.5, 1e-6));
        assert(near(ch0.peak, 0.5, 1e-6));
        assert(ch1.rms == 0.0f && ch1.peak == 0.0f);
    }

    // Silence floors at -200 dB, never -inf/NaN.
    {
        assert(near(toDbfs(0.0f), -200.0, 1e-3));
        assert(std::isfinite(toDbfs(0.0f)));
    }

    // Degenerate inputs return zeros instead of crashing.
    {
        const auto lv = computeLevels(nullptr, 100, 2, 0);
        assert(lv.rms == 0.0f && lv.peak == 0.0f);
        float x = 1.0f;
        const auto lv2 = computeLevels(&x, 1, 2, 5);  // channel out of range
        assert(lv2.rms == 0.0f && lv2.peak == 0.0f);
    }
